...
def get(config):
	...
	if config.dataset == 'clevr_veggies':
			dataset = make_clevr_veggies(config)
...
def make_clevr_veggies(config):
	...
	res = AttrDict(
			trainset=image.create(
					'clevr_veggies', subset='train', batch_size=batch_size, transforms=transform),
			validset=image.create(
					'clevr_veggies', subset='val', batch_size=batch_size, transforms=transform))
